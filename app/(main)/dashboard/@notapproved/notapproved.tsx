...
import { fetchNotApprovedShiftAction } from "@/actions/dashboard.actions";
...
export default function NotApproved({ initialData, pagination }: { initialData: Record[]; pagination: Pagination }) {
  const {
    displayedData,
    searchTerm,
    setSearchTerm,
    ...
    isSearching,
    loadMoreRef
  } = useInfiniteSearch<Record>(initialData, pagination, fetchNotApprovedShiftAction);
  ...
  return (
    <Card className="w-full h-[510px] border-border rounded-sm bg-card shadow-sm flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between px-7 py-1">
        <CardTitle className="text-[19px] font-bold">Not Approved Shifts</CardTitle>
