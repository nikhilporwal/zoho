...
import { fetchAbandonShiftAction } from "@/actions/dashboard.actions";
...
export default function Abandon({ initialData, pagination }: { initialData: Record[]; pagination: Pagination }) {
  const {
    displayedData,
    searchTerm,
    setSearchTerm,
    ...
    isSearching,
    loadMoreRef
  } = useInfiniteSearch<Record>(initialData, pagination, fetchAbandonShiftAction);
  ...
  return (
    <Card className="w-full h-[510px] border-border rounded-sm bg-card shadow-sm flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between px-7 py-1">
        <CardTitle className="text-[19px] font-bold">Abandon Shifts</CardTitle>
